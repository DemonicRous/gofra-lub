<template>
    <AppLayout>
                        ...
                        <div class="lg:col-span-2 space-y-6">
                            <div class="bg-gray-50 rounded-lg p-6">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <label class="text-sm text-gray-500">Отчество</label>
                                        <p class="font-medium">{{ user.patronymic || '—' }}</p>
                                    </div>
                                    <div>
                                        <label class="text-sm text-gray-500">Полное имя</label>
                                        <p class="font-medium">{{ user.full_name || '—' }}</p>
                                    </div>
                                    <div>
                                        <label class="text-sm text-gray-500">Короткое имя</label>
                                        <p class="font-medium">{{ user.short_name || '—' }}</p>
                                    </div>
                                    <div>
                                        <label class="text-sm text-gray-500">Никнейм</label>
                                        <p class="font-medium text-blue-600">@{{ user.nickname || '—' }}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="bg-gray-50 rounded-lg p-6">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <label class="text-sm text-gray-500">Должность</label>
                                        <p class="font-medium">{{ user.position || '—' }}</p>
                                    </div>
                                    <div>
                                        <label class="text-sm text-gray-500">Отдел</label>
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="bg-gray-50 rounded-lg p-6">
                                ...
                                <div class="space-y-3">
                                    ...
                                    <div class="flex items-center justify-between">
                                        <span class="text-sm text-gray-600">Аккаунт одобрен:</span>
                                        <span :class="user.approved_at ? 'text-green-600' : 'text-yellow-600'">
                                            {{ user.approved_at ? 'Да' : 'Нет' }}
                                        </span>
                                    </div>
                                    <div v-if="user.approved_at" class="pt-2 border-t border-gray-200">
                                        <span class="text-xs text-gray-500">Дата одобрения:</span>
                                        <p class="text-sm font-medium">{{ formatDate(user.approved_at) }}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="bg-gray-50 rounded-lg p-6">
                                ...
                                <div class="space-y-3">
                                    <div>
                                        <span class="text-xs text-gray-500">Дата регистрации:</span>
                                        <p class="text-sm font-medium">{{ formatDate(user.created_at) }}</p>
                                    </div>
                                    <div v-if="user.email_verified_at">
                                        <span class="text-xs text-gray-500">Email подтвержден:</span>
                                        <p class="text-sm font-medium">{{ formatDate(user.email_verified_at) }}</p>
                                    </div>
                                    <div v-if="user.updated_at !== user.created_at">
                                        <span class="text-xs text-gray-500">Последнее обновление:</span>
        ...
        <div v-if="showEditModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" @click.self="closeModal">
                ...
                <form @submit.prevent="updateUser" class="p-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <select v-model="editForm.role" class="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
